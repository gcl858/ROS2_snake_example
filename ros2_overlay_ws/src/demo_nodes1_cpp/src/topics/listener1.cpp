...
void print_usage()
{
  printf("Usage for listener1 app:\n");
  printf("listener1 [-t topic_name] [-h]\n");
  printf("options:\n");
  ...
  : Node("listener456")
  {
    ...
    auto callback =
      [this](const std_msgs::msg::String::SharedPtr msg) -> void
      {
        ...
        printf("try LEFT:%s",strstr( msg->data.c_str(),"LEFT"));
      };
...
int main(int argc, char * argv[])
{
  ...
  while (rclcpp::ok()){
    ...
    printf("start loop_rate.sleep()\n");
    loop_rate.sleep();
    printf("end loop_rate.sleep()\n");
  }

  /* // spin will block until work comes in, execute work as it becomes available, and keep blocking.
  // It will only be interrupted by Ctrl-C.
  printf("start rclcpp::spin(node)\n");
  rclcpp::spin(node);
  printf("wnd rclcpp::spin(node)\n"); */
